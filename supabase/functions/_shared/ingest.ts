...
import { invoiceTotalDollars, parseEinvoiceCsv } from "./einvoice_csv_parser.ts";
...
export async function ingestCsv(
  ...
  // categorized before keeps that category; the keyword categorizer is only the
  // fallback. Most-recent past choice wins.
  ...
  for (const p of parsed) {
    ...
    // Per item: its own history → keyword on the item name. Resolved first so the
    // header can fall back to their most common category.
    const itemCats = p.items.map((it) => {
...
// Mirrors lib/data/invoice_repository.dart and server/src/lib/ingest.ts. Scoped
// to userId by hand (the admin client bypasses RLS); the most-recent-wins
// reduction lives in category_resolver.
